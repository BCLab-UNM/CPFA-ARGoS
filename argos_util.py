...
PARAMETER_LIMITS = {
    ...
    "ProbabilityOfSwitchingToSearching": (0.3, 1),
    ...
    "UninformedSearchVariation": (0, 25)
}
...
def uniform_crossover(xml_file, p1_xml, p2_xml, rate, system="linux"):
    p1_parameters = copy.deepcopy(get_parameters(p1_xml))
    ...
    if flag:
        ...
        #child = default_argos_xml(system=system, time=parent_time, robots=parent_robots)
        child1 = default_argos_xml(xml_file, system=system, time=parent_time)
        set_parameters(child1, child1_parameters)
        child2 = default_argos_xml(xml_file, system=system, time=parent_time)
